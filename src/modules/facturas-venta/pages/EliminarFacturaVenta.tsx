...
import { deleteFacturaVenta,
         getFacturaVentaByNFactura }                        from '../'
import type { FacturaVentaCabecera }                        from '../'

export default function EliminarFacturaVenta() {
  const [nFactura, setNFactura]           = useState('')
  const [factura, setFactura]             = useState<FacturaVentaCabecera | null>(null)
  ...
  const {
    ...
    (id) => deleteFacturaVenta(Number(id)),
    'Factura de venta'
  )
  ...
  const handleBuscar = async () => {
    ...
    setBuscando(true)
    try {
      const data = await getFacturaVentaByNFactura(nFactura)
      setFactura(data)
    } catch (e: unknown) {
  ...
  return (
    <div className="flex flex-col items-center p-6 w-full">
      <h2 className="text-gray-200 text-2xl font-bold mb-6">Eliminar Factura de Venta</h2>
      ...
      {factura && !mensaje && (
        <div className="flex flex-col items-center gap-4 mt-4">
          <p className="text-gray-300 text-sm">
            ...
            {factura.razonSocial} ({factura.cifCliente})
          </p>
          {!confirmacion && (
