/**
 * Componente de formulario editable para crear o modificar un proveedor.
 */
...
export function ProveedorForm({
  ...
  return (
    <div className="grid grid-cols-[150px_1fr_150px_1fr] gap-x-4 gap-y-3 w-4/5 max-w-3xl items-center">
      ...
      {/* ID: sólo visible en modo edición, siempre readonly */}
      {form.idProveedor != null && (
        <Fragment>
          <label className={labelClass}>ID Proveedor</label>
          <input className={inputReadOnlyClass} readOnly value={form.idProveedor} />
        </Fragment>
      )}

      {/* ── Identificación ── */}
      <label htmlFor="descProveedor" className={labelClass}>Descripción <label className= {"text-red-400"}>*</label></label>
      <input
        id="descProveedor"
        className={inputClass}
        name="descProveedor"
        maxLength={300}
        value={form.descProveedor}
        onChange={onChange}
        onKeyDown={onKeyDown}
      />

      <label htmlFor="razonSocial" className={labelClass}>Razón Social <label className= {"text-red-400"}>*</label></label>
      <input
        id="razonSocial"
        className={inputClass}
        name="razonSocial"
      ...
      <label htmlFor="cifProveedor" className={labelClass}>CIF <label className= {"text-red-400"}>*</label></label>
      <input
        id="cifProveedor"
        className={inputClass}
        name="cifProveedor"
        maxLength={9}
        value={form.cifProveedor}
      ...
      <label htmlFor="ccProveedor" className={labelClass}>Cuenta Contable</label>
      <input
        id="ccProveedor"
        className={inputClass}
        name="ccProveedor"
        maxLength={10}
        value={form.ccProveedor}
        onChange={onChange}
        onKeyDown={onKeyDown}
      />

      <label htmlFor="dtoComercial" className={labelClass}>Dto. Comercial (%)</label>
      <input
        id="dtoComercial"
        className={inputClass}
        name="dtoComercial"
      ...
      <label htmlFor="observaciones" className={`${labelClass} w-[150px] shrink-0 pt-2`}>
          Observaciones
      </label>
      <textarea
          id="observaciones"
